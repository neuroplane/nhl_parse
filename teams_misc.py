...
#print(stats_parsed)
#print(sorted(stats_parsed, key=itemgetter('giveaways'), reverse=True))
#print(sorted(stats_parsed, key=itemgetter('missedShots'), reverse=True))
#print(sorted(stats_parsed, key=itemgetter('takeaways'), reverse=True))
stats_misc_parsed_hits_reduced = sorted(stats_misc_parsed, key=itemgetter('hits'), reverse=True)[:5]
stats_misc_parsed_blocks_reduced = sorted(stats_misc_parsed, key=itemgetter('blockedShots'), reverse=True)[:5]
stats_misc_parsed_missed_reduced = sorted(stats_misc_parsed, key=itemgetter('missedShots'), reverse=True)[:5]
stats_misc_parsed_giveaways_reduced = sorted(stats_misc_parsed, key=itemgetter('giveaways'), reverse=True)[:5]
stats_misc_parsed_takeaways_reduced = sorted(stats_misc_parsed, key=itemgetter('takeaways'), reverse=True)[:5]
stats_shots_parsed_shootingPct_reduced = sorted(stats_shots_parsed, key=itemgetter('shootingPct'), reverse=True)[:5]
stats_shots_parsed_shotsOnNet_reduced = sorted(stats_shots_parsed, key=itemgetter('shotsOnNet'), reverse=True)[:5]
stats_penalties_parsed_penaltyMinutes_reduced = sorted(stats_penalties_parsed, key=itemgetter('penaltyMinutes'), reverse=True)[:5]
...
with Image.open("pics/highlights.jpg") as im:
    ...
    row_1_start = 150
    row_2_start = 800
    row_3_start = row_2_start + 650
    row_4_start = row_3_start + 650
    ...
    for teams in  stats_misc_parsed_hits_reduced:
        #draw.text((pos_num, draw_start + LINE_HEIGHT), str(index), font=def_font, fill=GREY, anchor="rm")
